...
def aco(waypoints, n_ants=10, n_iterations=100, decay=0.5, alpha=1, beta=1, progress=True):
    ...
    itertimer = time.time()
    ctime = 0
    def showProgress(iteration=0, ant=0, check_time=False):
        nonlocal ctime, itertimer
        if check_time:
            ctime = toFixed(itertimer - time.time())
            itertimer = time.time()
            
        printProgress(iteration, n_iterations, " %s - %s/%s" % (ctime , ant, n_ants))
