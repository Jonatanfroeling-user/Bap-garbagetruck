...
const initWithMap = (MAP, locationData, routeData, currentUser, iconUserB) => {
  const { log } = console;
  ...
  function getIcon(coords, isPrimaryuser = true) {
    return new window.L.marker(coords, {
      icon: new LeafIcon({
        iconUrl: isPrimaryuser ? currentUser.truckIcon : iconUserB,
      }),
    }).addTo(MAP);
  }
  ...
  class ServerClass {
    ...
    assignRoute(name, routeId) {
      const user = JSON.copy(window._users[name]);
      const route = new RouteClass(routeId, user);
      window._users[name].activeRoute = routeId;
      ...
      return new DriverClass(name + Math.random(), user, route);
    }
  ...
  class DriverClass {
    constructor(id, userName, route) {
      this.id = id;
      this.name = userName;
      this.userName = userName;
      this.user = window._users[userName];
      ...
      this.truck = getIcon(mapCenter, userName === currentUser.name);
      this.done = false;
  ...
  // assing routes, would be normally done in beginning of day or on request or by admin
  const assignRoute = (userName, routeName) => {
    ...
    // reutrn new driver, should be done local somehow, maytbe just ahcgen name of this function
    return new DriverClass(userName + Math.random(), userName, route);
  };
  ...
  (async () => {
    ...
    for (let user of Object.values(window._users)) {
      console.log(user);
      if (user && user.name !== currentUser.name) {
        window._users[user.name] = assignRoute(user.name, "route-empty");
      }
    }

    window._users.currentUser = assignRoute(currentUser.name, "route-1");
